...
async function identifyIncompleteUsers() {
  ...
  try {
    ...
    if (noProfileResult.rows.length > 0) {
      console.table(noProfileResult.rows.map(row => ({
        Email: row.email,
    ...
    if (incompleteProfileResult.rows.length > 0) {
      console.table(incompleteProfileResult.rows.map(row => ({
        Email: row.email,
    ...
    const totalIncomplete = noProfileResult.rows.length + incompleteProfileResult.rows.length
    ...
    console.log('   "Complete Your IIIT NR Alumni Profile - Now Easier Than Ever!"')
    console.log('')
    ...
    console.log('')
    console.log('3. When they log in, they\'ll auto-redirect to /complete-profile')
    console.log('')
    console.log('4. Consider: Users inactive for 6+ months = send final reminder')
