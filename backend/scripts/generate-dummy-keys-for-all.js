...
(async () => {
  try {
    ...
    console.log(
      "⚠️  Real users should generate their own keys in the browser.\n",
    );
    ...
    console.log(
      `Found ${usersWithoutKeys.rows.length} users without public keys:\n`,
    );
    ...
    if (parseInt(users_without_keys) === 0) {
      ...
      console.log(
        "Users should visit the Messages page to generate real keys.",
      );
      console.log(
        "Real keys will automatically replace dummy keys on first Messages page visit.",
      );
    }
