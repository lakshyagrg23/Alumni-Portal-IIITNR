...
async function checkEncryptionKeys() {
  ...
  try {
    ...
    if (!tableCheck.rows[0].exists) {
      ...
      console.log(
        "   Run migration: database/migrations/001_messaging_essential.sql",
      );
      return;
    }
    ...
    `);
    console.log(
      `🔐 Users with encrypted private keys: ${usersWithEncrypted.rows[0].count}\n`,
    );
    ...
    if (samples.rows.length === 0) {
      ...
      samples.rows.forEach((row, idx) => {
        ...
        console.log(
          `      Encrypted private key length: ${row.enc_priv_key_length || "NULL"}`,
        );
    ...
    const coverage =
      totalUsers.rows[0].count > 0
        ? (
            (usersWithKeys.rows[0].count / totalUsers.rows[0].count) *
            100
          ).toFixed(1)
        : "0.0";
    console.log(
      `   Key coverage: ${coverage}% (${usersWithKeys.rows[0].count}/${totalUsers.rows[0].count} users)`,
    );

    if (usersWithKeys.rows[0].count < totalUsers.rows[0].count) {
      console.log(
        `   ⚠️  ${totalUsers.rows[0].count - usersWithKeys.rows[0].count} users need to log in to generate keys`,
      );
    }
  } catch (error) {
