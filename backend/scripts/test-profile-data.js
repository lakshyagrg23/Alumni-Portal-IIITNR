  ...
  try {
    ...
    profiles.rows.forEach((row, i) => {
      ...
      if (row.first_name) {
        ...
        console.log(
          `   Consent: ${row.consent_for_accreditation ? "Yes" : "No"}`,
        );
      } else {
    ...
    console.log(
      `With profiles: ${profiles.rows.filter((r) => r.first_name).length}`,
    );
    console.log(
      `Without profiles: ${profiles.rows.filter((r) => !r.first_name).length}`,
    );
