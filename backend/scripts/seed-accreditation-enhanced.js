...
async function seedAccreditationData() {
  ...
  try {
    ...
    const alumniResult = await client.query(
      "SELECT id FROM alumni_profiles LIMIT 10",
    );
    const alumniIds = alumniResult.rows.map((row) => row.id);

    if (alumniIds.length === 0) {
      console.log(
        "⚠️  No alumni profiles found. Please seed alumni data first.",
      );
    ...
    for (let i = 0; i < 5; i++) {
      const alumniId = alumniIds[Math.floor(Math.random() * alumniIds.length)];
      const type =
        contributionTypes[Math.floor(Math.random() * contributionTypes.length)];
    ...
    for (let i = 0; i < 8; i++) {
      const alumniId = alumniIds[Math.floor(Math.random() * alumniIds.length)];
      const type =
        achievementTypes[Math.floor(Math.random() * achievementTypes.length)];
