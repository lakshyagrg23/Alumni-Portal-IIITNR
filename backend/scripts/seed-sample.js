    ...
    for (const alumni of sampleAlumni) {
      try {
        ...
        if (existingProfile.rows.length === 0) {
          ...
          console.log(
            `   ✅ Created profile for ${alumni.firstName} ${alumni.lastName}`,
          );
        } else {
          console.log(
            `   ⚠️  Profile already exists for ${alumni.firstName} ${alumni.lastName}`,
          );
        }
      } catch (error) {
        console.error(
          `   ❌ Failed to create profile for ${alumni.firstName} ${alumni.lastName}:`,
          error.message,
        );
      }
    }
