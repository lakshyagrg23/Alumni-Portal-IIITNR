...
const { default: pool, query, closePool } = await import('./src/config/database.js')
...
async function sendReengagementEmails() {
  ...
  try {
    ...
    if (incompleteUsers.length === 0) {
      console.log('✅ No incomplete users found! All verified alumni have completed onboarding.')
      return
    }
    ...
    // Categorize users
    const noProfile = incompleteUsers.filter(u => u.category === 'no_profile')
    const incompleteProfile = incompleteUsers.filter(u => u.category === 'incomplete_profile')

    console.log(`   • ${noProfile.length} verified but never started onboarding`)
    console.log(`   • ${incompleteProfile.length} started but didn't complete\n`)

    // Show sample of who will receive emails
    console.log('📧 Recipients:')
    console.table(incompleteUsers.slice(0, 10).map(u => ({
      Email: u.email,
    ...
    for (let i = 0; i < incompleteUsers.length; i++) {
      ...
      try {
        ...
        if (result.success) {
          ...
          console.log(`${progress} ❌ Failed to send to ${user.email}: ${result.error}`)
        }
        ...
        console.log(`${progress} ❌ Error sending to ${user.email}: ${error.message}`)
      }
    }
    ...
    console.log('   4. For failed emails, verify email addresses are correct\n')
