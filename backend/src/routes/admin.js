  ...
  async (req, res) => {
    try {
      ...
      const stats = {
        ...
        instituteEmails: users.filter((user) =>
          user.email.endsWith("@iiitnr.edu.in")
        ).length,
        externalEmails: users.filter(
          (user) => !user.email.endsWith("@iiitnr.edu.in")
        ).length,
      };
  ...
  try {
    if (req.user.role !== "superadmin") {
      ...
      const has = await query(
        ...
        [req.user.id, "manage_news"]
      );
      if (!has.rowCount)
        return res
          .status(403)
          .json({ success: false, message: "Missing permission: manage_news" });
    }
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const result = await query("DELETE FROM news WHERE id = $1 RETURNING *", [
        id,
      ]);
...
router.get("/permissions/me", async (req, res) => {
  try {
    ...
    const perms = await query(
      "SELECT permission FROM admin_permissions WHERE user_id=$1 ORDER BY permission",
      [req.user.id]
    );
...
router.get("/superadmin/admins", requireSuperadminAuth, async (req, res) => {
  try {
    ...
    if (ids.length) {
      const perms = await query(
        "SELECT user_id, permission FROM admin_permissions WHERE user_id = ANY($1)",
        [ids]
      );
...
router.post("/superadmin/demote", requireSuperadminAuth, async (req, res) => {
  try {
    ...
    if (user.role === "superadmin")
      return res
        .status(400)
        .json({ success: false, message: "Cannot demote superadmin" });
    ...
    res.json({
      success: true,
      message: "User demoted to alumni",
      data: updated,
    });
  } catch (e) {
    ...
    try {
      ...
      if (user.role !== "admin")
        return res
          ...
          .json({
            success: false,
            message: "Permissions can be granted only to admin users",
          });
      await query(
        "INSERT INTO admin_permissions(user_id, permission, granted_by) VALUES ($1,$2,$3) ON CONFLICT(user_id, permission) DO NOTHING",
        [userId, String(permission), req.user.id]
      );
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to grant permission" });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to revoke permission" });
    }
  }
);
    ...
    if (req.user.role !== "superadmin") {
      ...
      if (!has.rowCount)
        return res
          ...
          .json({
            success: false,
            message: "Missing permission: manage_events",
          });
    }
  ...
  try {
    ...
    if (!title || !description || !startDatetime || !endDatetime) {
      return res.status(400).json({
        success: false,
        message:
          "Title, description, start datetime, and end datetime are required",
      });
    }
...
/**
 * Accreditation Dashboard V2 API Routes
 * Batch-centric reporting for realistic accreditation metrics
 */

/**
 * @route   GET /api/admin/accreditation/overview
 * @desc    Get overview statistics for accreditation dashboard
 * @access  Admin only
 */
router.get("/accreditation/overview", async (req, res) => {
  try {
    console.log("[ACCREDITATION] Overview endpoint hit");
    console.log("[ACCREDITATION] Query params:", req.query);
    console.log(
      "[ACCREDITATION] User:",
      req.user?.email,
      "Role:",
    ...
    console.log(
      "[ACCREDITATION] Calling getOverviewStats with:",
      startYear,
      endYear
    );

    const stats = await accreditationQueries.getOverviewStats(
      parseInt(startYear),
      parseInt(endYear)
    );

    console.log(
      "[ACCREDITATION] Stats retrieved:",
    ...
    console.error("[ACCREDITATION] Get overview error:", error);
    console.error("[ACCREDITATION] Error stack:", error.stack);
    res.status(500).json({
      success: false,
      message: "Server error while fetching overview statistics",
      error: error.message,
    });
  }
});
...
/**
 * @route   GET /api/admin/accreditation/batch-coverage
 * @desc    Get registration coverage by batch
 * @access  Admin only
 */
router.get("/accreditation/batch-coverage", async (req, res) => {
  try {
    console.log("[ACCREDITATION] Batch coverage endpoint hit");
    const { startYear = 2015, endYear = 2025 } = req.query;
    const coverage = await accreditationQueries.getBatchCoverage(
      parseInt(startYear),
      parseInt(endYear)
    );

    console.log(
      "[ACCREDITATION] Batch coverage retrieved:",
      coverage.length,
      "rows"
    );
    ...
    console.error("[ACCREDITATION] Get batch coverage error:", error);
    console.error("[ACCREDITATION] Error stack:", error.stack);
    res.status(500).json({
      success: false,
      message: "Server error while fetching batch coverage",
      error: error.message,
    });
  }
});
...
/**
 * @route   GET /api/admin/accreditation/employment-outcomes
 * @desc    Get employment outcomes by batch (alumni only)
 * @access  Admin only
 */
router.get("/accreditation/employment-outcomes", async (req, res) => {
  try {
    const { startYear = 2015, endYear = 2024 } = req.query;
    const outcomes = await accreditationQueries.getEmploymentOutcomes(
      parseInt(startYear),
      parseInt(endYear)
    );
    ...
    console.error("Get employment outcomes error:", error);
    res.status(500).json({
      success: false,
      message: "Server error while fetching employment outcomes",
    });
  }
});

/**
 * @route   GET /api/admin/accreditation/employment-summary
 * @desc    Get employment status summary (overall distribution)
 * @access  Admin only
 */
router.get("/accreditation/employment-summary", async (req, res) => {
  try {
    const { startYear = 2015, endYear = 2024 } = req.query;
    const summary = await accreditationQueries.getEmploymentSummary(
      parseInt(startYear),
      parseInt(endYear)
    );
    ...
    res.status(500).json({
      success: false,
      message: "Server error while fetching employment summary",
    });
  }
});

/**
 * @route   GET /api/admin/accreditation/top-industries
 * @desc    Get top industries where alumni work
 * @access  Admin only
 */
router.get("/accreditation/top-industries", async (req, res) => {
  try {
    const { startYear = 2015, endYear = 2024, limit = 10 } = req.query;
    const industries = await accreditationQueries.getTopIndustries(
      parseInt(startYear),
      parseInt(endYear),
      parseInt(limit)
    );
    ...
    res.status(500).json({
      success: false,
      message: "Server error while fetching top industries",
    });
  }
});

/**
 * @route   GET /api/admin/accreditation/top-companies
 * @desc    Get top companies where alumni are employed
 * @access  Admin only
 */
router.get("/accreditation/top-companies", async (req, res) => {
  try {
    const { startYear = 2015, endYear = 2024, limit = 15 } = req.query;
    const companies = await accreditationQueries.getTopCompanies(
      parseInt(startYear),
      parseInt(endYear),
      parseInt(limit)
    );
    ...
    res.status(500).json({
      success: false,
      message: "Server error while fetching top companies",
    });
  }
});

/**
 * @route   GET /api/admin/accreditation/geographic-distribution
 * @desc    Get geographic distribution of alumni
 * @access  Admin only
 */
router.get("/accreditation/geographic-distribution", async (req, res) => {
  try {
    const { startYear = 2015, endYear = 2024, limit = 15 } = req.query;
    const distribution = await accreditationQueries.getGeographicDistribution(
      parseInt(startYear),
      parseInt(endYear),
      parseInt(limit)
    );
    ...
    res.status(500).json({
      success: false,
      message: "Server error while fetching geographic distribution",
    });
  }
});

/**
 * @route   GET /api/admin/accreditation/profile-quality
 * @desc    Get profile quality metrics
 * @access  Admin only
 */
router.get("/accreditation/profile-quality", async (req, res) => {
  try {
    const { startYear = 2015, endYear = 2024 } = req.query;
    const quality = await accreditationQueries.getProfileQualityMetrics(
      parseInt(startYear),
      parseInt(endYear)
    );
    ...
    console.error("Get profile quality metrics error:", error);
    res.status(500).json({
      success: false,
      message: "Server error while fetching profile quality metrics",
    });
  }
});
